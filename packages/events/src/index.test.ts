...
describe('InMemoryEventBus', () => {
  it('publishes only to subscribers of the matching in-process event type', async () => {
    ...
    const seen: string[] = [];
    bus.subscribe('network.changed', (published) => {
      seen.push(published.type);
    });
    bus.subscribe('dns.changed', (published) => {
      seen.push(`wrong:${published.type}`);
    });
  ...
  it('unsubscribes handlers and isolates later publishes from cleaned-up listeners', async () => {
    const bus = new InMemoryEventBus();
    const seen: string[] = [];
    const unsubscribe = bus.subscribe('network.changed', (published) => {
      seen.push(published.id);
    });

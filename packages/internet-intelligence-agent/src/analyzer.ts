...
export function analyzeInternetEvidence(
  ...
  if (!current.internetReachable) {
    if (current.dnsLookupMs === null) {
      return recommendation(
        ...
        'prefer_resolver',
        0.92,
        'Internet reachability failed and DNS evidence is unavailable.',
        ['internet-unreachable', 'dns-unavailable'],
        now,
      );
    }
    if (current.httpsHandshakeMs === null) {
      return recommendation(
        ...
        'recheck_destination',
        0.88,
        'DNS evidence exists but HTTPS handshake evidence is unavailable.',
        ['dns-present', 'tls-unavailable'],
        now,
      );
    }
    return recommendation(
      ...
      'defer_to_decision_engine',
      0.82,
      'The gateway is reachable but the destination is not reachable.',
  ...
  if (current.packetLossRatio >= 0.2) {
    ...
    return recommendation(
      'packet_loss',
      'prefer_path',
      clamp(0.7 + current.packetLossRatio),
      'Packet loss is materially elevated.',
      evidence,
      now,
    );
  }
  if (current.httpsHandshakeMs === null && current.httpResponseMs !== null) {
    return recommendation(
      ...
      'recheck_destination',
      0.86,
      'HTTP evidence exists without a successful TLS handshake.',
      ['http-present', 'tls-missing'],
      now,
    );
  }
  if (current.dnsLookupMs !== null && current.dnsLookupMs >= 1500) {
    return recommendation(
      ...
      'prefer_resolver',
      0.84,
      'DNS resolution latency is materially elevated.',
      [`dns-latency-ms:${current.dnsLookupMs}`],
      now,
    );
  }
  if (current.latencyMs !== null && current.latencyMs >= 250) {
    ...
    return recommendation(
      ...
      'prefer_path',
      0.8,
      'End-to-end latency is materially elevated.',
      evidence,
      now,
    );
  }
  if (current.ipv4Connectivity && !current.ipv6Connectivity) {
    return recommendation(
      ...
      'defer_to_decision_engine',
      0.78,
      'IPv4 is healthy while IPv6 connectivity is unavailable.',
      ['ipv4-ok', 'ipv6-unavailable'],
      now,
    );
  }
  ...
  if (degraded >= 3) {
    return recommendation(
      ...
      'defer_to_decision_engine',
      0.76,
      'Repeated degraded observations indicate a persistent path-specific problem; this is not proof of filtering.',
      ['persistent-degradation', `samples:${recent.length}`],
      now,
    );
  }
  ...
  return recommendation(
    ...
    'observe',
    0.95,
    'No material network anomaly was detected in the supplied evidence.',
    ['reachability-ok', 'loss-normal', 'latency-normal'],
    now,
  );
}

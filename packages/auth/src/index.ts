...
export interface Principal { id: string; roles: string[]; scopes: string[]; organizationId?: string; metadata?: Record<string, unknown>; }
...
export interface AuthorizationContext { principal: Principal | null; resource: string; action: string; requiredPermissions?: string[]; }
...
export interface JwtClaims { sub: string; roles: string[]; scopes: string[]; organizationId?: string; sessionId?: string; type: 'access' | 'refresh' | 'email-verification' | 'password-reset'; iat: number; exp: number; jti: string; }
const b64url = (input: Buffer | string): string => Buffer.from(input).toString('base64url');
const parseJson = <T>(value: string): T => JSON.parse(Buffer.from(value, 'base64url').toString('utf8')) as T;
export class JwtService {
  constructor(private readonly secret: string, private readonly issuer = 'internet-resilience-platform') { if (secret.length < 32) throw new Error('JWT secret must be at least 32 characters.'); }
  sign(input: Omit<JwtClaims, 'iat' | 'exp' | 'jti'> & { ttlSeconds: number }): string {
    const now = Math.floor(Date.now() / 1000);
    const header = { alg: 'HS256', typ: 'JWT' };
    const payload = { iss: this.issuer, ...input, iat: now, exp: now + input.ttlSeconds, jti: randomUUID() };
...
export class JwtAuthenticationProvider implements AuthenticationProvider {
  ...
  async authenticate(request: AuthenticationRequest): Promise<Principal | null> {
    ...
    return { id: claims.sub, roles: claims.roles, scopes: claims.scopes, ...(claims.organizationId ? { organizationId: claims.organizationId } : {}), metadata: { sessionId: claims.sessionId, jti: claims.jti } };
  }
}
export class RbacAuthorization implements AuthorizationMiddleware {
  async authorize(context: AuthorizationContext): Promise<boolean> {
    ...
    return context.requiredPermissions.every((permission) => context.principal?.scopes.includes(permission));
  }
}
export const hashPassword = (password: string): string => { const salt = randomBytes(16).toString('hex'); return `scrypt$${salt}$${scryptSync(password, salt, 64).toString('hex')}`; };
export const verifyPassword = (password: string, hash: string): boolean => { const [, salt, digest] = hash.split('$'); if (!salt || !digest) return false; const candidate = scryptSync(password, salt, 64); return timingSafeEqual(candidate, Buffer.from(digest, 'hex')); };

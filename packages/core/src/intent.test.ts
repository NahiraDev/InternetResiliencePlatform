...
describe('network intent lifecycle', () => {
  ...
  it('rejects invalid transitions', () => {
    expect(() => transitionIntent(base(), { type: 'complete' })).toThrow(
      /Invalid intent transition/,
    );
  });
  ...
  it('preserves the replacement identity when superseding', () => {
    ...
    const superseded = transitionIntent(active, { type: 'supersede', replacementId: 'intent-2' });

...
export interface NetworkIntent {
  ...
  readonly spec: NetworkIntentSpec;
...
const TRANSITIONS: Readonly<Record<IntentStatus, readonly IntentStatus[]>> = {
  ...
  active: ['completed', 'superseded', 'cancelled', 'expired'],
  completed: [],
  superseded: [],
  cancelled: [],
  expired: [],
...
const iso = (value: string | undefined, field: string): string | undefined => {
  if (value === undefined) return undefined;
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) throw new RangeError(`${field} must be a valid ISO-8601 timestamp`);
...
export const createNetworkIntent = (
  input: Omit<NetworkIntent, 'version' | 'status' | 'createdAt' | 'updatedAt'> &
    Partial<Pick<NetworkIntent, 'createdAt' | 'updatedAt'>>,
): NetworkIntent => {
  ...
  if (effectiveFrom && expiresAt && effectiveFrom >= expiresAt) {
    throw new RangeError('effectiveFrom must be earlier than expiresAt');
  }
...
export const transitionIntent = (intent: NetworkIntent, command: IntentCommand): NetworkIntent => {
  const target: IntentStatus = command.type === 'activate'
    ? 'active'
  ...
  if (TERMINAL.has(intent.status)) {
    throw new Error(`Cannot transition terminal intent ${intent.id} from ${intent.status}`);
  }
  if (!TRANSITIONS[intent.status].includes(target)) {
    throw new Error(`Invalid intent transition ${intent.status} -> ${target}`);
  }
  ...
  return Object.freeze({
    ...
    status: target,
    updatedAt,
    ...(command.type === 'supersede' ? { supersedes: command.replacementId } : {}),
  });
};

export const isIntentTerminal = (intent: NetworkIntent): boolean => TERMINAL.has(intent.status);

export const isIntentEffective = (intent: NetworkIntent, at = new Date()): boolean => {

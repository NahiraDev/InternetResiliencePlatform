...
const withTimeout = async <T>(timeoutMs: number, signal: AbortSignal, fn: () => Promise<T>): Promise<T> => { if (signal.aborted) throw abortError(); return new Promise<T>((resolve, reject) => { let timer: NodeJS.Timeout; const cleanup = () => { clearTimeout(timer); signal.removeEventListener('abort', onAbort); }; const onAbort = () => { cleanup(); reject(abortError()); }; signal.addEventListener('abort', onAbort, { once: true }); timer = setTimeout(() => { signal.removeEventListener('abort', onAbort); reject(new Error('operation timed out')); }, timeoutMs); void fn().then((v) => { cleanup(); resolve(v); }, (e) => { cleanup(); reject(e); }); }); };

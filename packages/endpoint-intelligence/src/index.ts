...
export class EndpointRegistry {
  ...
  observe(observation: EndpointObservation): EndpointHealth {
    ...
    const latencyScore = latencySamples.length
      ? latencySamples.reduce((sum, item) => sum + observationScore(item), 0) / latencySamples.length
      : availabilityScore;
  ...
  list(filter?: { status?: EndpointStatus; region?: string; protocol?: EndpointProtocol }): EndpointRecord[] {
  ...
  rank(options: { region?: string; protocol?: EndpointProtocol; limit?: number } = {}): EndpointRecord[] {
    const limit = Math.max(1, Math.floor(options.limit ?? 20));
    return this.list(options)
      .filter((endpoint) => endpoint.status !== 'retired')
      .sort((a, b) => {
        const score = (record: EndpointRecord) =>
          record.health.reliabilityScore * 0.5 +
          record.health.availabilityScore * 0.35 +
          record.health.confidence * 0.15;

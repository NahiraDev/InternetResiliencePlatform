...
import { EndpointRegistry, scoreEndpoint } from './index.js';
...
describe('EndpointRegistry', () => {
  it('registers an endpoint without sharing mutable state', () => {
    const registry = new EndpointRegistry({ now: () => new Date('2026-08-19T00:00:00.000Z') });
    const record = registry.register(endpoint);
    record.tags?.push('mutated');

    expect(registry.size()).toBe(1);
    expect(registry.get(endpoint.id)?.tags).toBeUndefined();
    expect(registry.get(endpoint.id)?.health.status).toBe('new');
  });

  it('aggregates observations and produces a healthy endpoint', () => {
  ...
  it('marks persistently failing endpoints as retired', () => {
  ...
  it('calculates a bounded aggregate score', () => {
    expect(scoreEndpoint({
      endpointId: endpoint.id,
      status: 'healthy',

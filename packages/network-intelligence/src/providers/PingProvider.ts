...
export interface PingProvider {
  ping(host: string, signal: AbortSignal): Promise<PingResult>;
}
/** Deterministic in-process fixture for unit tests. */
export class MockablePingProvider implements PingProvider {
  async ping(_host: string, signal: AbortSignal): Promise<PingResult> {
    signal.throwIfAborted();
    return { latencyMs: 1, success: true };
  }
}
...
export class SystemPingProvider implements PingProvider {
  constructor(private readonly timeoutMs = 2_000) {
    if (!Number.isInteger(timeoutMs) || timeoutMs < 250)
      throw new Error('timeoutMs must be an integer >= 250');
  }
  async ping(host: string, signal: AbortSignal): Promise<PingResult> {
    if (!host.trim() || /[\r\n]/.test(host))
      throw new Error('host must be a non-empty single-line value');
    signal.throwIfAborted();
    const args =
    ...
    try {
      ...
      return {
        success: true,
        latencyMs: Number.isFinite(value) ? value : performance.now() - started,
      };
    } catch (error) {

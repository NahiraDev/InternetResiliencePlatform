...
describe('PingProvider', () => {
  it('keeps the deterministic mock fixture available', async () => {
    const result = await new MockablePingProvider().ping(
      'fixture.local',
  ...
  it('rejects empty and multiline hosts before invoking the OS utility', async () => {
    ...
    await expect(provider.ping('  ', new AbortController().signal)).rejects.toThrow(
      'host must be a non-empty single-line value',
    );
    await expect(
      provider.ping('example.com\necho unsafe', new AbortController().signal),
    ).rejects.toThrow('host must be a non-empty single-line value');
  });

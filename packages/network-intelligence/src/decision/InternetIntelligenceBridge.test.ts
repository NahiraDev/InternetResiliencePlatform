...
import {
  ...
  type InternetIntelligenceAdvisor,
  type NetworkDecisionContextWithInternetEvidence,
} from './InternetIntelligenceBridge.js';
...
describe('InternetIntelligenceBridge', () => {
  it('passes canonical evidence without inventing measurements', async () => {
    ...
    await expect(new InternetIntelligenceBridge(advisor).analyze(context)).resolves.toEqual(
      recommendation,
    );
  ...
  it('does not call the advisor when evidence is absent', async () => {
    ...
    const contextWithoutEvidenceTyped: Omit<
      NetworkDecisionContextWithInternetEvidence,
      'internetEvidence'
    > = contextWithoutEvidence;
    await expect(
      new InternetIntelligenceBridge(advisor).analyze(contextWithoutEvidenceTyped),
    ).resolves.toBeNull();
    expect(calls).toBe(0);
  });
  ...
  it('returns only advisory fields through the model-provider boundary', async () => {
    const provider = new InternetIntelligenceBridge({
      observe: async () => recommendation,
    }).modelProvider();

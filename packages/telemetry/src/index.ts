...
export const bootstrapOpenTelemetry = (
...
): { tracerName: string; enabled: boolean; exporterConfigured: boolean } => {
  if (!config.enabled)
    return { tracerName: config.serviceName, enabled: false, exporterConfigured: false };
  ...
  return {
    ...
    enabled: true,
    exporterConfigured: Boolean(config.otlpEndpoint),
  };
};
...
export const extractTraceContext = (carrier: Record<string, string | string[] | undefined>) =>
...
export const observeDependency = async <T>(
  dependency: string,
  operation: string,
  fn: () => Promise<T>,
): Promise<T> => {
  ...
  try {
    ...
    dependencyLatencyMs.observe(
      { dependency, operation, status: 'success' },
      performance.now() - started,
    );
    return value;
  } catch (error) {
    dependencyLatencyMs.observe(
      { dependency, operation, status: 'failure' },
      performance.now() - started,
    );
...
export const classifyError = (error: unknown): ErrorCategory => {
  ...
  if (message.includes('authenticat') || message.includes('credential') || message.includes('jwt'))
...
export const probeSuccessTotal = new client.Counter({
  name: 'irp_probe_success_total',
  help: 'Successful network probe executions',
  labelNames: ['probe_type', 'probe_name'],
  registers: [prometheusRegister],
});
export const probeFailureTotal = new client.Counter({
  name: 'irp_probe_failure_total',
  help: 'Failed network probe executions',
  labelNames: ['probe_type', 'probe_name'],
  registers: [prometheusRegister],
});
export const networkLatencyMs = new client.Histogram({
  name: 'irp_network_latency_ms',
  help: 'Network probe latency in milliseconds',
  labelNames: ['probe_type', 'probe_name'],
  registers: [prometheusRegister],
});

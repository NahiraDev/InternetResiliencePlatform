...
export class ResilienceRuntime {
  ...
  async cycle(input: Partial<RuntimeContext> & { idempotencyKey?: string } = {}) {
    ...
    try { const record = await this.inFlight; if (input.idempotencyKey) this.idempotency.set(input.idempotencyKey, record); return record; }
    ...
    try {
      ...
      if (context.mode === 'simulation') {
        ...
        if (verification.status === 'failed') {
          ...
          // A recovery operation is not itself proof of recovered network state.
          outcome = 'degraded';
  ...
  private async recordBlocked(context: RuntimeContext, before: RuntimeContext['mode'] | string, observations: any, found: any, candidates: any, plan: any, start: number, validation?: any) {

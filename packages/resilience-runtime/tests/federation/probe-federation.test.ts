...
const evidence = (
  probeId: string,
  status: 'reachable' | 'degraded' | 'unreachable' = 'reachable',
) => ({
...
describe('ProbeFederation', () => {
  ...
  it('compares observations from multiple regions', () => {
    const a = createProbeKeyPair(),
      b = createProbeKeyPair(),
  ...
  it('rejects revoked probes', () => {
    ...
    expect(
      federation.ingest(signProbeEvidence(evidence('probe-1'), keys.privateKeyPem)).reason,
    ).toBe('revoked-probe');
  });
});

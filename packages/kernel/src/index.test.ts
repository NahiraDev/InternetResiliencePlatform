...
import { CapabilityError, DIContainer, FeatureFlags, KernelRuntime, StateMachine, createContract } from './index.js';
...
describe('network kernel', () => {
  it('enforces contract capabilities through the kernel execution pipeline', async () => {
    ...
    kernel.registerContract(createContract({ namespace: 'dns', version: '1.0.0', operations: { resolve: { capability: 'dns.resolve', execute: (input) => ({ input, records: ['127.0.0.1'] }) } } }));

...
describe('network kernel', () => {
  it('enforces contract capabilities through the kernel execution pipeline', async () => {
    ...
    await expect(
      kernel.execute(
        ...
        'resolve',
        { name: 'example.com' },
        { principal: { id: 'plugin', capabilities: [] } },
      ),
    ...
    await expect(
      kernel.execute(
        ...
        'resolve',
        { name: 'example.com' },
        { principal: { id: 'plugin', capabilities: ['dns.resolve'] } },
      ),
    ).resolves.toMatchObject({ records: ['127.0.0.1'] });
  });
  ...
  it('runs every middleware again on message retry', async () => {
    ...
    let attempts = 0;
    kernel.bus.use(async (message, next) => {
      middlewareCalls.push(`outer-${message.attempts}`);
      return next();
    });
    kernel.bus.use(async (message, next) => {
      middlewareCalls.push(`inner-${message.attempts}`);
      return next();
    });
    kernel.bus.on('Retryable', () => {
      attempts += 1;
  ...
  it('requires every configuration migration step', () => {
    ...
    config.set('network', {
      ...
      migrations: {},
      compatibleWith: [1],
      validate: (data) => (data.mode ? [] : ['mode required']),
    });
    expect(() => config.migrate('network', 2)).toThrowError(KernelError);
    config.set('network', {
      schemaVersion: 1,
      data: { mode: 'safe' },
      migrations: { 1: (data) => ({ ...(data as { mode: string }), timeoutMs: 5000 }) },
      compatibleWith: [1],
      validate: (data) => (data.mode ? [] : ['mode required']),
    });
  ...
  it('runs workflows in simulation mode without executing actions', async () => {
    const kernel = new KernelRuntime();
    let executed = false;
    kernel.bus.on('vpn.connect', () => {
      executed = true;
    });
    const result = await kernel.workflows.run(
      {
        id: 'connect',
        trigger: 'manual',
        steps: [{ id: 'vpn', capability: 'vpn.connect', action: 'vpn.connect' }],
      },
      kernel.context({ id: 'ai', capabilities: ['vpn.connect'] }),
      true,
    );
  ...
  it('manages feature flags and typed state machines', () => {
    ...
    const vpn = new StateMachine('disconnected', [
      { from: 'disconnected', to: 'connected', event: 'connect' },
    ] as const);

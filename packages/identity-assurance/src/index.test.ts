...
import { assessIdentityPolicy, type IdentityEvidence } from './index.js';
...
describe('assessIdentityPolicy', () => {
  ...
  it('does not infer service identity from egress identity', () => {
    const result = assessIdentityPolicy(evidence(), {
  ...
  it('rejects an unauthorized egress even when the destination is allowed', () => {
    const result = assessIdentityPolicy(evidence(), {
  ...
  it('reports missing evidence explicitly', () => {
    ...
    expect(result.status).toBe('insufficient-data');
    expect(result.findings.map((finding) => finding.code)).toEqual(['missing-egress', 'missing-destination']);
  });
});

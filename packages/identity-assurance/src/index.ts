...
export const assessIdentityPolicy = (
  ...
  if (!evidence) {
    return {
      ...
      destination: null,
      findings: [
        { code: 'missing-egress', message: 'independent egress identity evidence is required' },
        { code: 'missing-destination', message: 'destination identity evidence is required' },
      ],
      evaluatedAt,
    };
  }
  ...
  if (egressAge < 0 || destinationAge < 0) findings.push({ code: 'stale-evidence', message: 'evidence timestamp is in the future' });
  if (egressAge > maxAge || destinationAge > maxAge) findings.push({ code: 'stale-evidence', message: `identity evidence exceeds the ${maxAge}ms freshness window` });
  if (evidence.confidence === 'insufficient') findings.push({ code: 'insufficient-confidence', message: 'evidence confidence is insufficient for policy assurance' });
  ...
  const hasEgressConstraint = allowedIps.length > 0 || allowedAsns.length > 0 || allowedOrganizations.length > 0;
  if (hasEgressConstraint) {
  ...
  const hasDestinationConstraint = allowedHostnames.length > 0 || allowedAddresses.length > 0;
  if (hasDestinationConstraint) {
  ...
  const insufficient = findings.some((finding) => finding.code === 'missing-egress' || finding.code === 'missing-destination' || finding.code === 'stale-evidence' || finding.code === 'insufficient-confidence');

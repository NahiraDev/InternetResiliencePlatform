...
export class WireGuardProvider implements TunnelProvider {
  ...
  private readonly addressCidr: string | undefined;
  ...
  constructor(options: WireGuardProviderOptions) {
    ...
    if (!Number.isInteger(this.commandTimeoutMs) || this.commandTimeoutMs <= 0)
      throw new Error('WireGuard commandTimeoutMs must be a positive integer');
    if (!Number.isInteger(this.handshakeMaxAgeMs) || this.handshakeMaxAgeMs <= 0)
      throw new Error('WireGuard handshakeMaxAgeMs must be a positive integer');
    if (!Number.isInteger(this.startupTimeoutMs) || this.startupTimeoutMs <= 0)
      throw new Error('WireGuard startupTimeoutMs must be a positive integer');
    if (!Number.isInteger(this.pollIntervalMs) || this.pollIntervalMs <= 0)
      throw new Error('WireGuard pollIntervalMs must be a positive integer');
  }
  capabilitiesInfo() {
    return {
      protocols: ['wireguard'],
  ...
  async create(config: TunnelConfiguration): Promise<Tunnel> {
    ...
    if (config.endpoint.protocol !== 'wireguard')
      throw tunnelErrors.unsupported('WireGuard provider requires a wireguard endpoint protocol');
    if (config.authentication.type !== 'key')
      throw tunnelErrors.auth('WireGuard requires key-based authentication');
    if (!config.credentialRef)
      throw tunnelErrors.configuration(
        'WireGuard requires a credential reference for the private key',
      );
  ...
  async connect(tunnel: Tunnel): Promise<TunnelConnection> {
    ...
    try {
      ...
      try {
        await this.runWgSet(
          keyFile.path,
          tunnel.configuration.keepalive.intervalMs,
          tunnel.endpoint,
        );
      } finally {
      ...
      if (this.addressCidr)
        await this.runIp(['address', 'replace', this.addressCidr, 'dev', this.interfaceName]);
      ...
      if (health.status !== 'healthy') {
        ...
        throw new TunnelError(
          'WireGuard interface started but did not provide a fresh handshake before the startup deadline',
          'WireGuardHealthCheckFailed',
          'dependencyFailure',
          true,
      ...
      if (createdInterface) {
        try {
          await this.runIp(['link', 'del', 'dev', this.interfaceName]);
        } catch {
  ...
  async disconnect(connection: TunnelConnection, _timeoutMs: number): Promise<void> {
    const entry = [...this.runtime.entries()].find(
      ([, value]) => value.connectionId === connection.id,
    );
    if (!entry) return;
    try {
      await this.runIp(['link', 'del', 'dev', entry[1].interfaceName]);
    } finally {
      this.runtime.delete(entry[0]);
    }
  }
  ...
  async healthCheck(tunnel?: Tunnel): Promise<TunnelHealth> {
    ...
    try {
      const show = await this.commandRunner.run(
        WG_COMMAND,
        ['show', runtime.interfaceName, 'latest-handshakes'],
        { timeoutMs: this.commandTimeoutMs },
      );
      if (show.exitCode !== 0)
        throw new Error(show.stderr || 'WireGuard latest-handshakes query failed');
      const latestHandshake = parseLatestHandshake(show.stdout);
      const handshakeAgeMs =
        latestHandshake === undefined
          ? Number.POSITIVE_INFINITY
          : Math.max(0, Date.now() - latestHandshake * 1000);
      const handshake = handshakeAgeMs <= this.handshakeMaxAgeMs;
      const state = await this.commandRunner.run(
        IP_COMMAND,
        ['link', 'show', 'dev', runtime.interfaceName],
        { timeoutMs: this.commandTimeoutMs },
      );
      const up = state.exitCode === 0 && /\bstate UP\b|<[^>]*UP[^>]*>/.test(state.stdout);
      return {
        status: up && handshake ? 'healthy' : up ? 'degraded' : 'unhealthy',
        connectivity: up,
        handshake,
  ...
  static async generateKeyPair(
    ...
    if (privateResult.exitCode !== 0)
      throw new Error(privateResult.stderr || 'WireGuard private key generation failed');
    const privateKey = privateResult.stdout.trim();
    assertKey(privateKey, 'private key');
    const publicResult = await commandRunner.run(WG_COMMAND, ['pubkey'], {
      stdin: `${privateKey}\n`,
    });
    if (publicResult.exitCode !== 0)
      throw new Error(publicResult.stderr || 'WireGuard public key derivation failed');
  ...
  private async loadPrivateKey(credentialRef: string | undefined): Promise<string> {
    if (!credentialRef)
      throw tunnelErrors.auth('WireGuard private key credential reference is required');
  ...
  private async ensureInterface(): Promise<boolean> {
    const result = await this.commandRunner.run(
      IP_COMMAND,
      ['link', 'show', 'dev', this.interfaceName],
      { timeoutMs: this.commandTimeoutMs },
    );
    if (result.exitCode === 0)
      throw tunnelErrors.configuration(
        `WireGuard interface ${this.interfaceName} already exists and is not owned by this provider`,
      );
    await this.runIp(['link', 'add', 'dev', this.interfaceName, 'type', 'wireguard']);
    return true;
  }
  private async runWgSet(
    ...
    const keepaliveSeconds =
      this.peer.persistentKeepalive ??
      (keepaliveMs > 0 ? Math.max(1, Math.min(65535, Math.round(keepaliveMs / 1000))) : 0);
    const args = [
      'set',
      this.interfaceName,
      'private-key',
      ...
      this.peer.publicKey,
      'allowed-ips',
      this.peer.allowedIPs.join(','),
      'endpoint',
      peerEndpoint,
    ];
    if (keepaliveSeconds > 0) args.push('persistent-keepalive', String(keepaliveSeconds));
    const result = await this.commandRunner.run(WG_COMMAND, args, {
      timeoutMs: this.commandTimeoutMs,
    });
  ...
  private async runIp(args: string[]): Promise<void> {
    const result = await this.commandRunner.run(IP_COMMAND, args, {
      timeoutMs: this.commandTimeoutMs,
    });
    if (result.exitCode !== 0) throw new Error(result.stderr || `ip ${args.join(' ')} failed`);
  }
  private async createPrivateKeyFile(
    privateKey: string,
  ...
  private async waitForHealthy(tunnel: Tunnel): Promise<TunnelHealth> {
    ...
    while (last.status !== 'healthy' && Date.now() < deadline) {
      await new Promise((resolve) =>
        setTimeout(resolve, Math.min(this.pollIntervalMs, Math.max(1, deadline - Date.now()))),
      );
  ...
  private assertTunnel(tunnel: Tunnel): void {
    if (tunnel.providerId !== this.id)
      throw tunnelErrors.configuration('Tunnel is not owned by WireGuard provider');
    if (tunnel.endpoint.protocol !== 'wireguard')
      throw tunnelErrors.unsupported('Tunnel endpoint is not WireGuard');
    if (tunnel.configuration.authentication.type !== 'key')
      throw tunnelErrors.auth('WireGuard requires key-based authentication');
    if (!tunnel.configuration.credentialRef)
      throw tunnelErrors.auth('WireGuard tunnel is missing private-key credential reference');
  }
}
function assertKey(value: string, label: string): void {
  if (!KEY_PATTERN.test(value))
    throw new TunnelError(
      `invalid WireGuard ${label}`,
      'WireGuardInvalidKey',
...
function assertPeer(peer: WireGuardPeerConfig): void {
  ...
  if (
    ...
    (!Number.isInteger(peer.persistentKeepalive) ||
      peer.persistentKeepalive < 0 ||
      peer.persistentKeepalive > 65535)
  )
...
function formatEndpoint(endpoint: Endpoint): string {
  const host =
    endpoint.addressFamily === 'ipv6' && !endpoint.host.startsWith('[')
      ? `[${endpoint.host}]`
      : endpoint.host;
  return `${host}:${endpoint.port}`;
}
function parseLatestHandshake(output: string): number | undefined {
  const values = output
    .split(/\s+/)
    .map(Number)
...
function sanitizeWireGuardError(error: unknown): Error {
  ...
  return new TunnelError(
    message.replace(KEY_PATTERN, '[REDACTED_KEY]'),
    'WireGuardOperationFailed',
    'dependencyFailure',
    true,
  );
}

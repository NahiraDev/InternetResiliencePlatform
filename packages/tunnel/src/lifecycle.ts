...
export class AutomatedTunnelLifecycle {
  ...
  async rotate(
    ...
    try {
      ...
      const configuration: TunnelConfiguration = {
        ...
        ...changes,
        endpoint: changes.endpoint ?? current.configuration.endpoint,
        authentication: changes.authentication ?? current.configuration.authentication,
        credentialRef: changes.credentialRef ?? current.configuration.credentialRef,
      };
  ...
  private requiresKillSwitch(tunnel: Tunnel): boolean {
    return this.options.killSwitchRequired || tunnel.configuration.securityProfile === 'strict' || tunnel.configuration.routingMode === 'fullTunnel';
  }
  ...
  private async markFailed(tunnelId: string, error: unknown): Promise<void> {
    ...
    try {
      if (tunnel.state !== 'failed') this.tunnels.set(tunnelId, transitionTunnel(tunnel, 'failed'));
    } catch {
  ...
  private lock(tunnelId: string): void {
    if (this.locks.has(tunnelId)) throw tunnelErrors.state('Concurrent tunnel lifecycle operation rejected', { tunnelId });

...
export interface TunnelProvider {
  ...
  disconnect(connection: TunnelConnection, timeoutMs: number): Promise<void>;
...
export class UnsupportedProtocolProvider implements TunnelProvider {
  ...
  async disconnect(_connection: TunnelConnection, _timeoutMs: number): Promise<void> {}
...
export class TunnelManager implements RecoveryTunnelActions {
  ...
  async disconnectTunnel(tunnelId: string): Promise<void> {
    ...
    try {
      ...
      if (connection) await provider.disconnect(connection, 30_000);

...
const boundedLimit = (limit: number | undefined): number =>
  Math.min(Math.max(1, Math.floor(limit ?? DEFAULT_MAX_SAMPLES)), MAX_QUERY_SAMPLES);
...
const validateMeasurement = (measurement: HistoricalMeasurement): void => {
  ...
  if (
    ...
    (!Number.isFinite(measurement.packetLossPercent) ||
      measurement.packetLossPercent < 0 ||
      measurement.packetLossPercent > 100)
  )
...
const summarize = (measurements: readonly HistoricalMeasurement[]): HistoricalSummary => {
  const latency = measurements.flatMap((item) =>
    item.latencyMs === undefined ? [] : [item.latencyMs],
  );
  const packetLoss = measurements.flatMap((item) =>
    item.packetLossPercent === undefined ? [] : [item.packetLossPercent],
  );
  const successCount = measurements.filter((item) => item.success).length;
  return {
    sampleCount: measurements.length,
    successCount,
    availabilityPercent: measurements.length
      ? round((successCount / measurements.length) * 100)
      : null,
...
const bucketDurationMs = (size: BucketSize): number =>
  size === 'hour'
    ? HOUR_MS
    : size === 'six-hour'
      ? 6 * HOUR_MS
...
const buildBuckets = (
  ...
  for (let start = from; start < to; start += duration) {
    ...
    const items = measurements.filter((item) => {
...
const calculateTrend = (
  ...
  const points = measurements.flatMap((item) => {
    ...
    return value === undefined || !Number.isFinite(value)
      ? []
      : [{ x: Date.parse(item.timestamp), y: value }];
  });
  if (points.length < 2)
    return {
      metric,
  ...
  const changePercent =
    Math.abs(first) > EPSILON ? round(((last - first) / Math.abs(first)) * 100) : null;
...
export class InMemoryHistoricalMeasurementStore implements HistoricalMeasurementStore {
  ...
  constructor(measurements: readonly HistoricalMeasurement[] = []) {
    measurements.forEach((measurement) => this.add(measurement));
  }
  add(measurement: HistoricalMeasurement): void {
    validateMeasurement(measurement);
    this.measurements.push(cloneMeasurement(measurement));
  }
  async query(query: HistoricalQuery): Promise<HistoricalMeasurement[]> {
    ...
    return this.measurements
      .filter((item) => {
        const timestamp = Date.parse(item.timestamp);
        return (
          timestamp >= from && timestamp < to && (!probeTypes || probeTypes.has(item.probeType))
        );
      })
...
export class PostgresHistoricalMeasurementStore implements HistoricalMeasurementStore {
  ...
  async query(query: HistoricalQuery): Promise<HistoricalMeasurement[]> {
    ...
    if (!Array.isArray(rows))
      throw new Error('historical measurement query returned an invalid result');
    const probeTypes = query.probeTypes ? new Set(query.probeTypes) : undefined;
...
export const createHistoricalReport = async (
  ...
  const measurements = (
    await store.query({ ...query, limit: Math.min(query.limit ?? maxSamples, maxSamples) })
  )
    .map(cloneMeasurement)
    .sort((a, b) => Date.parse(a.timestamp) - Date.parse(b.timestamp));
  ...
  return {
    ...
    series: probeTypes.map((probeType) =>
      createSeries(
        probeType,
        measurements.filter((item) => item.probeType === probeType),
        from,
        to,
        bucketSize,
      ),

...
export type GatewayFailoverState =
  'idle' | 'selecting' | 'switching' | 'verifying' | 'succeeded' | 'exhausted';
export interface GatewayFailoverEvent {
  type:
    ...
    | 'gateway.failover.rollback_succeeded'
    | 'gateway.failover.rollback_failed'
...
export interface GatewayFailoverExecutor {
  switchGateway(
    candidate: GatewaySelectionCandidate,
    reason: string,
  ): Promise<GatewayFailoverVerification>;
  restoreGateway?(gatewayId: string, reason: string): Promise<GatewayFailoverVerification>;
}
...
function assertOptions(options: Required<GatewayFailoverOptions>): void {
  if (
    !Number.isInteger(options.maxFailovers) ||
    options.maxFailovers < 1 ||
    options.maxFailovers > 10
  )
    throw new Error('maxFailovers must be an integer between 1 and 10');
  if (
    !Number.isInteger(options.quarantineMs) ||
    options.quarantineMs < 0 ||
    options.quarantineMs > 3_600_000
  )
    ...
    try {
      const baseline = selectGateway({
        ...
        ...(request.currentGatewayId !== undefined
          ? { currentGatewayId: request.currentGatewayId }
          : {}),
        policy: { ...DEFAULT_GATEWAY_SELECTION_POLICY, ...(request.policy ?? {}) },
        now,
      });
      ...
      if (
        ...
        return {
          ...
          reason:
            'Failover was not required because the current gateway remains eligible and healthy enough under policy.',
        };
      }
      ...
      const candidates = baseline.candidates.filter(
        (candidate) =>
          candidate.eligible &&
          !excluded.has(candidate.gateway.id) &&
          !this.isQuarantined(candidate.gateway.id, nowMs),
      );
      ...
      for (
        let index = 0;
        index < Math.min(candidates.length, this.options.maxFailovers);
        index += 1
      ) {
        ...
        await this.emit({
          type: 'gateway.failover.candidate',
        ...
        try {
          ...
          if (verification.healthy) {
            ...
            await this.emit({
              type: 'gateway.failover.switch_succeeded',
              gatewayId: candidate.gateway.id,
              currentGatewayId: request.currentGatewayId,
              attempt,
              reason: verification.reason ?? 'gateway switch verified healthy',
            ...
            await this.emit({
              type: 'gateway.failover.succeeded',
              gatewayId: candidate.gateway.id,
              currentGatewayId: request.currentGatewayId,
              attempt,
          ...
          if (request.currentGatewayId && this.executor.restoreGateway) {
            try {
              const restored = await this.executor.restoreGateway(
                request.currentGatewayId,
                `rollback after failed switch to ${candidate.gateway.id}: ${failureReason}`,
              );
              rolledBack = restored.healthy;
              await this.emit({
                type: restored.healthy
                  ? 'gateway.failover.rollback_succeeded'
                  : 'gateway.failover.rollback_failed',
                gatewayId: request.currentGatewayId,
                currentGatewayId: candidate.gateway.id,
                attempt,
                reason:
                  restored.reason ??
                  (restored.healthy
                    ...
                    : 'previous gateway restoration failed'),
                occurredAt: new Date().toISOString(),
              });
            } catch (error) {
              await this.emit({
                type: 'gateway.failover.rollback_failed',
                gatewayId: request.currentGatewayId,
                currentGatewayId: candidate.gateway.id,
                attempt,
          ...
          attempts.push({
            ...
            verified: false,
            rolledBack,
            reason: `${failureReason}${request.currentGatewayId && this.executor.restoreGateway ? (rolledBack ? '; previous gateway restored' : '; previous gateway restoration failed') : '; rollback adapter unavailable'}`,
          });
          this.quarantinedUntil.set(candidate.gateway.id, nowMs + this.options.quarantineMs);
          await this.emit({
            type: 'gateway.failover.switch_failed',
            gatewayId: candidate.gateway.id,
            currentGatewayId: request.currentGatewayId,
            attempt,
          ...
          const failureReason =
            error instanceof Error ? error.message : 'gateway switch execution failed';
          attempts.push({
            gatewayId: candidate.gateway.id,
            attempt,
            selectedScore: candidate.score,
      ...
      await this.emit({
        type: 'gateway.failover.exhausted',
        currentGatewayId: request.currentGatewayId,
        reason: finalReason,
        occurredAt: new Date().toISOString(),
      });

...
describe('Starlink resource catalog', () => {
  it('contains every supplied resource without treating sample endpoints as public configs', () => {
    ...
    expect(STARLINK_RESOURCES.every((resource) => resource.publicEndpointProvided === false)).toBe(
      true,
    );
  ...
  it('supports capability and protocol discovery', () => {
    ...
    expect(wireGuard.map((resource) => resource.id)).toEqual(
  ...
  it('never exposes a live endpoint merely because a provider has a displayed example IP', () => {
    ...
    expect(egret?.notes.some((note) => note.includes('Displayed IP addresses are examples'))).toBe(
      true,
    );
  });
  ...
  it('keeps catalog entries immutable from callers', () => {
    ...
    expect(
      listStarlinkResources({ protocol: 'openvpn' }).some(
        (resource) => resource.id === 'getastatic',
      ),
    ).toBe(false);
  });
});

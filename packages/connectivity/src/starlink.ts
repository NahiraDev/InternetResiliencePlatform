...
const STARLINK_CAPABILITIES: ConnectivityCapability[] = [
  'monitor',
  'health-check',
  'supports-ipv4',
  'supports-ipv6',
...
export class StarlinkDishClient {
  ...
  constructor(options: StarlinkProviderOptions = {}) {
    ...
    if (!/^\[[0-9a-fA-F:]+\]:\d+$|^[^:]+:\d+$/.test(this.target)) {
      throw new Error('Invalid Starlink gRPC target');
    }
    if (!Number.isInteger(this.timeoutMs) || this.timeoutMs <= 0) {
      throw new Error('Starlink timeoutMs must be positive');
    }
  }

  async probe(): Promise<boolean> {
    const separator = this.target.lastIndexOf(':');
    const host = this.target.startsWith('[')
      ? this.target.slice(1, this.target.indexOf(']'))
      : this.target.slice(0, separator);
  ...
  async getStatus(
    ...
    try {
      return parseStatus(JSON.parse(result.stdout) as Record<string, unknown>);
    } catch {
      return undefined;
    }
  }
...
export class StarlinkProvider implements ConnectivityProvider {
  ...
  private readonly commandRunner: StarlinkCommandRunner;
  ...
  constructor(options: StarlinkProviderOptions = {}) {
    ...
    this.commandRunner = options.commandRunner ?? new NodeStarlinkCommandRunner();
    ...
    if (
      !Number.isInteger(this.minimumHealthyScore) ||
      this.minimumHealthyScore < 0 ||
      this.minimumHealthyScore > 100
    ) {
      throw new Error('Starlink minimumHealthyScore must be 0..100');
    }
  }

  capabilities(): ConnectivityCapability[] {
    return [...STARLINK_CAPABILITIES];
  }
  ...
  async discover(): Promise<ConnectivityResource[]> {
    ...
    const resource: ConnectivityResource = {
      providerId: this.id,
      id: this.resourceId,
      type: this.type,
      state: !reachable
  ...
  async getHealth(resourceId = this.resourceId): Promise<ConnectivityHealth> {
    ...
    const status = await this.client.getStatus(this.commandRunner);
    ...
    if (status.latencyMs !== undefined) {
      health.latencyMs = status.latencyMs;
    }

    const packetLoss = normalizePacketLoss(status.packetLoss);
    if (packetLoss !== undefined) {
      health.packetLoss = packetLoss;
    }

    if (status.downloadMbps !== undefined) {
      health.bandwidthMbps = status.downloadMbps;
    }
  ...
  async connect(resourceId: string): Promise<ConnectivityOperationResult> {
    ...
    if (health.status === 'healthy' || health.status === 'degraded') {
      return {
        ...
        metadata: { operation: 'verify-uplink' },
      };
    }
    ...
    return {
      ...
      resourceId,
      state: 'failed',
      error: 'Starlink dish is not reachable',
    };
  }
  ...
  async disconnect(resourceId: string): Promise<ConnectivityOperationResult> {
    ...
    return {
      ...
      resourceId,
      state: 'active',
      error: 'Starlink provider does not own dish power or link lifecycle',
    };
  }
  ...
  private assertResource(resourceId: string): void {
    if (resourceId !== this.resourceId) {
      throw new Error(`Unknown Starlink resource: ${resourceId}`);
    }
  }
}
...
function parseStatus(root: Record<string, unknown>): StarlinkStatusSnapshot {
  ...
  const download = bitsToMbps(
    firstNumber(status.downlink_throughput_bps, status.downlink_throughput),
  );
  ...
  const obstruction =
    firstNumber(status.obstruction_percent) ??
    (obstructionFraction === undefined ? undefined : obstructionFraction * 100);
  if (obstruction !== undefined) snapshot.obstructionPercent = obstruction;

  const uptime = firstNumber(status.uptime_seconds, status.uptime);
  if (uptime !== undefined) snapshot.uptimeSeconds = uptime;

  return snapshot;
}

function calculateScore(status: StarlinkStatusSnapshot): number {
  let score = 100;
  const packetLoss = normalizePacketLoss(status.packetLoss);

  if (packetLoss !== undefined) {
    score -= Math.min(45, packetLoss * 3);
  }
  if (status.latencyMs !== undefined) {
...
function normalizePacketLoss(value: number | undefined): number | undefined {

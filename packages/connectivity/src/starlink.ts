...
export class StarlinkProvider implements ConnectivityProvider {
  ...
  constructor(options: StarlinkProviderOptions = {}) {
    ...
    if (!Number.isInteger(this.minimumHealthyScore) || this.minimumHealthyScore < 0 || this.minimumHealthyScore > 100)
    ...
    return [
      {
        ...
        metadata: {
          vendor: 'SpaceX',
          technology: 'Starlink',
          localApiTarget: '192.168.100.1:9200',
  ...
  async getHealth(resourceId = this.resourceId): Promise<ConnectivityHealth> {
    ...
    return {
      ...
      packetLoss: normalizePacketLoss(status.packetLoss),
...
function parseStatus(root: Record<string, unknown>): StarlinkStatusSnapshot {
  ...
  return {
    ...
    obstructionPercent: firstNumber(status.obstruction_percent) ??
...
function calculateScore(status: StarlinkStatusSnapshot): number {
  ...
  if (packetLoss !== undefined) score -= Math.min(45, packetLoss * 3);

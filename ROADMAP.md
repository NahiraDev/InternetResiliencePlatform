...
- **Plugin-first** — anything that *can* be a plugin, *is* a plugin (VPN backends, proxy tools, notification channels, etc.).
...
### Phase 12 — Quality Prediction Engine
**Objective:** Forecast degradation before it's user-visible using recent trend data.
**Deliverables:** Trend analysis on rolling metrics, threshold-based prediction, alert-before-drop hooks.

...
function metricsText() {
  const lines = ['# HELP irp_runtime_lab_up Runtime lab process health.', '# TYPE irp_runtime_lab_up gauge', `irp_runtime_lab_up ${shuttingDown ? 0 : 1}`];
...
async function sendTrace(spans) {
  try { await fetch(TEMPO_URL, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ resourceSpans: [{ resource: { attributes: [attr('service.name', 'irp-runtime-lab'), attr('service.version', '0.1.0')] }, scopeSpans: [{ scope: { name: 'irp-runtime-lab', version: '1.0.0' }, spans }] }] }), signal: AbortSignal.timeout(5000) }); }
  catch (error) { inc('irp_observability_export_failures_total', { signal: 'traces' }); console.error(JSON.stringify({ level: 'error', event: 'trace_export_failed', error: String(error) })); }
}
...
async function executeScenario() {
  ...
  try {
    const registrySpan = span('gateway-registry.select', rootSpanId, 1, 2, { 'irp.source': 'runtime-lab', 'irp.target': '@irp/gateway-registry' }); inc('irp_package_calls_total', { source: 'runtime-lab', target: 'gateway-registry', operation: 'select' });
    const { selectGateway } = await import('../../packages/gateway-registry/dist/selection.js'); const nowDate = new Date();
    ...
    const selection = selectGateway({ gateways: [gateway('gw-a', 'tehran'), gateway('gw-b', 'qazvin')], health }); inc('irp_gateway_selections_total'); inc('irp_gateway_selection_success_total', { gateway: selection.selected?.gateway.id ?? 'none' }); setGauge('irp_gateway_selected_score', {}, selection.selected?.score ?? 0); span('gateway-registry.evaluate', registrySpan, 0.1, 0.8, { 'irp.gateway.selected': selection.selected?.gateway.id ?? 'none', 'irp.gateway.score': selection.selected?.score ?? 0 });
    const runtimeSpan = span('resilience-runtime.validation', rootSpanId, 4, 8, { 'irp.source': 'runtime-lab', 'irp.target': '@irp/resilience-runtime' }); inc('irp_package_calls_total', { source: 'runtime-lab', target: 'resilience-runtime', operation: 'phase40-validation' });
    const { runPhase40Validation } = await import('../../packages/resilience-runtime/dist/e2e-validation.js'); const report = await runPhase40Validation(); const passed = report.status === 'passed'; inc('irp_runtime_cycles_total'); if (!passed) inc('irp_runtime_cycles_failed_total'); inc('irp_runtime_scenarios_total', { status: report.status }); setGauge('irp_runtime_acceptance_criteria', {}, Object.values(report.acceptance).filter(Boolean).length); span('resilience-runtime.scenarios', runtimeSpan, 0.2, 6, { 'irp.scenarios': report.scenarios.length, 'irp.status': report.status }, passed ? 1 : 2);
    const elapsedMs = Number(process.hrtime.bigint() - started) / 1e6; setGauge('irp_runtime_cycle_duration_ms', {}, elapsedMs); root.endTimeUnixNano = String(now + BigInt(Math.round(elapsedMs * 1_000_000))); lastReport = { generatedAt: new Date().toISOString(), status: report.status, deterministic: report.deterministic, scenarios: report.scenarios, acceptance: report.acceptance, failedCriteria: report.failedCriteria, gatewaySelection: { selected: selection.selected?.gateway.id ?? null, score: selection.selected?.score ?? null, candidates: selection.candidates.map((c) => ({ id: c.gateway.id, eligible: c.eligible, score: c.score })) }, durationMs: elapsedMs, traceId }; console.log(JSON.stringify({ level: 'info', event: 'runtime_cycle', trace_id: traceId, status: report.status, gateway: selection.selected?.gateway.id, failed_criteria: report.failedCriteria, duration_ms: elapsedMs }));
  } catch (error) { const message = error instanceof Error ? error.message : String(error); const elapsedMs = Number(process.hrtime.bigint() - started) / 1e6; inc('irp_runtime_cycles_failed_total'); root.status = { code: 2, message }; root.endTimeUnixNano = String(now + BigInt(Math.round(elapsedMs * 1_000_000))); lastReport = { generatedAt: new Date().toISOString(), status: 'failed', deterministic: true, scenarios: [], acceptance: {}, failedCriteria: ['runtime-cycle-exception'], error: message, traceId, durationMs: elapsedMs }; console.error(JSON.stringify({ level: 'error', event: 'runtime_cycle_failed', trace_id: traceId, error: message })); }
...
async function shutdown(signal) {
  if (shuttingDown) return; shuttingDown = true; if (scenarioTimer) { clearTimeout(scenarioTimer); scenarioTimer = null; }
  setGauge('irp_runtime_scenario_running', {}, 0); console.log(JSON.stringify({ level: 'info', event: 'shutdown_requested', signal }));
  await Promise.all([closeServer(app, 'lab'), closeServer(metrics, 'metrics')]); appListening = false; metricsListening = false;
}
for (const signal of ['SIGTERM', 'SIGINT']) process.once(signal, () => { void shutdown(signal).finally(() => process.exit(0)); });

await Promise.all([listen(app, PORT, 'lab'), listen(metrics, METRICS_PORT, 'metrics')]); appListening = true; metricsListening = true; await executeScenario(); scheduleScenario();

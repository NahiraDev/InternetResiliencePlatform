...
function metricsText() {
  const lines = [
    ...
    `irp_runtime_lab_up ${shuttingDown ? 0 : 1}`,
  ];
...
function recordPackageReport(code, stderr) {
  packageReport = readPackageReport() ?? {
    schemaVersion: 1,
    overall: code === 0 ? 'healthy' : 'unhealthy',
    totals: {},
    packages: [],
  };
  packageReport.generatedAt ??= new Date().toISOString();
...
function runPackageAudit() {
  ...
  const child = spawn(process.execPath, ['tools/runtime-lab/package-integration.mjs'], {
    cwd: ROOT,
    env: process.env,
  ...
  child.on('close', (code) => {
    ...
    recordPackageReport(code ?? 1, stderr);
    publish('package-audit', packageReport);
  });
}

async function executeScenario() {
  if (scenarioRunning || shuttingDown) {
    if (scenarioRunning) inc('irp_runtime_cycles_skipped_total', { reason: 'already_running' });
    return;
  }
...
function scheduleScenario() {
  ...
  scenarioTimer = setTimeout(() => {
    void executeScenario().finally(scheduleScenario);
  }, SCENARIO_INTERVAL_MS);
}

function schedulePackageAudit() {
  if (packageAuditTimer) clearTimeout(packageAuditTimer);
  if (shuttingDown) return;
  packageAuditTimer = setTimeout(() => {
    runPackageAudit();
    schedulePackageAudit();
  }, PACKAGE_AUDIT_INTERVAL_MS);
}
...
const app = http.createServer((req, res) => {
  ...
  if (url.pathname === '/health') {
    ...
    res.end(JSON.stringify({ status: 'ok', service: 'irp-runtime-lab', shuttingDown }));
    return;
  }
...
async function shutdown(signal) {
  ...
  clients.clear();
  await Promise.all([closeServer(app, 'lab'), closeServer(metrics, 'metrics')]);
  appListening = false;
  metricsListening = false;
...
// Do not await long-running runtime work from module scope. Node 24 exits with
// code 13 when a top-level await remains unsettled; the HTTP servers must stay
// independently alive while package audits and runtime scenarios execute.
runPackageAudit();

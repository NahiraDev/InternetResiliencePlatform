...
async function runApiE2E(pkg) {
  ...
  try {
    ...
    if (parsedVersion?.success !== true || !parsedVersion?.data?.version)
    ...
    const metricsOk = /irp_http_requests_total|http_requests_total|http_request_total/.test(
      metricsText,
    );
    if (!metricsOk) {
      const snippet = metricsText.slice(0, 2000);
      throw new Error(
        `metrics response contract failed; metrics did not include expected metric. metrics_snippet=${JSON.stringify(snippet)}`,
      );
    }
...
async function runDaemonE2E(pkg) {
  ...
  if (!pkg.entryPath || !existsSync(pkg.entryPath))
    return {
      ...
      state: 'failed',
      mode: 'process-e2e',
      error: `entry not found at ${pkg.entryPath}`,
    };
  ...
  try {
    const result = await new Promise((resolveResult) => {
      ...
      runtime.child.once('exit', (code, signal) => {
        clearTimeout(timer);
        settle({ ok: false, code, signal, output: runtime.getOutput() });
      });
      runtime.child.once('error', (error) => {
        clearTimeout(timer);
        settle({ ok: false, error: error.message, output: runtime.getOutput() });
      });
    });
...
async function runLibraryImport(pkg, byName) {
  ...
  if (!pkg.entryPath || !existsSync(pkg.entryPath))
    return {
      ...
      mode: 'manifest-only',
      latencyMs: Math.round(performance.now() - started),
      reason: pkg.entryPath ? `entry not found at ${pkg.entryPath}` : 'no entry point defined',
    };
  const load = await runNode('await import(process.argv[1])', [
    new URL(`file://${pkg.entryPath}`).href,
  ]);
  const integrations = [];
  for (const targetName of pkg.workspaceDeps.filter((name) => byName.has(name))) {
    ...
    const check = await runNode('await import(process.argv[1]); await import(process.argv[2])', [
      new URL(`file://${pkg.entryPath}`).href,
  ...
  return {
    ...
    directory: pkg.directory,
    state: load.ok
      ? integrations.some((item) => item.state === 'failed')
        ? 'degraded'
        : 'executed'
      : 'failed',
    mode: 'runtime-import',
    ...
    error: load.stderr || load.error || undefined,
    integrations,
  };
}
...
const report = {
  ...
  overall: runtime.some((item) => item.state === 'failed')
    ? 'unhealthy'
    : runtime.some((item) => item.state === 'degraded')
      ? 'degraded'
      : 'healthy',
  totals: {
    packages: results.length,
    runtime: runtime.length,
    executed: results.filter((item) => item.state === 'executed').length,
...
if (process.argv.includes('--strict')) {
  if (report.overall !== 'healthy') {
    ...
    console.error(
      `Totals: ${report.totals.packages} packages, ${report.totals.runtime} runtime, ${report.totals.executed} executed, ${report.totals.failed} failed, ${report.totals.degraded} degraded\n`,
    );

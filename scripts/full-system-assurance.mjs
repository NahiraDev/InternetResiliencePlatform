...
const contract = JSON.parse(
  await readFile(join(root, 'ops/release/full-system-assurance.json'), 'utf8'),
);
const registry = JSON.parse(
  await readFile(join(root, contract.realEnvironmentAdapterRegistry), 'utf8'),
);
const outputDir = join(
  root,
  process.env.IRP_FULL_SYSTEM_OUTPUT ?? 'artifacts/full-system-assurance',
);
const rows = [];
...
async function walk(dir) {
  ...
  for (const entry of await readdir(dir, { withFileTypes: true })) {
    ...
    if (
      entry.isDirectory() &&
      !['node_modules', '.git', 'dist', 'build', '.turbo'].includes(entry.name)
    )
...
function capabilityFor(id) {
  ...
  const rules = [
    ['resilience-runtime', 'resilience-runtime'],
    ...
    ['ios/packettunnel', 'ios-network-extension'],
  ...
  if (value.includes('ops/observability') || value.includes('observability'))
...
function executableEntrypoint(abs, manifest) {
  ...
  if (manifest?.bin)
    return `package.bin:${typeof manifest.bin === 'string' ? manifest.bin : Object.values(manifest.bin)[0]}`;
  return null;
}
...
function realEnvironmentContract(capability) {
  ...
  if (!entry)
    return {
      ...
      reason: 'No registered real-environment assurance contract',
    };
  return {
    status: 'PENDING',
    assurancePath: contract.realEnvironmentAdapterRegistry,
    evidenceIds: entry.evidenceIds ?? [],
    environment: entry.environment ?? null,
    failureInjection:
      'Required by evidence schema; adapter must record the injected degradation/failure.',
    recoveryVerification:
      'Required by evidence schema; adapter must verify recovery on the real path.',
    telemetryEvidence:
      'Required by evidence schema; adapter must provide traceId and observed telemetry.',
  };
}
...
async function addSurface(path, type, packageJson = null) {
  ...
  const sourceFiles = files.filter((file) =>
    /\.(ts|tsx|js|mjs|cjs|swift|kt|kts|sh|sql)$/.test(file),
  ).length;
  ...
  rows.push({
    ...
    sourceFiles,
    upstreamDependencies: packageJson
      ? Object.keys({
...
for (const top of ['packages', 'apps']) {
  ...
  for (const entry of await readdir(join(root, top), { withFileTypes: true })) {
    ...
    await addSurface(
      path,
      top === 'apps' ? 'app' : 'package',
      existsSync(manifest) ? await readJson(manifest) : null,
    );
  }
}
...
if (existsSync(workflowDir))
  for (const file of await readdir(workflowDir))
    if (/\.(yml|yaml)$/.test(file)) await addSurface(`.github/workflows/${file}`, 'workflow');
...
for (const row of packageRows)
  row.downstreamDependencies = [...(reverse.get(row.path.split('/').at(-1)) ?? [])].sort();
...
const canonicalPhaseRecords = [
  ...phasePolicyText.matchAll(/\[`?phase-(\d+)\.md`?\]\(phase-\1\.md\)/g),
...
for (let n = 0; n <= contract.roadmap.maxPhase; n++)
  rows.push({
    componentId: `roadmap::phase-${n}`,
    path: `docs/phases/phase-${String(n).padStart(2, '0')}.md`,
    type: 'roadmap-phase',
    ...
    owningPhases: [n],
    realEnvironment: {
      status: canonicalPhaseSet.includes(n)
        ? missingCanonicalPhaseDocs.includes(n)
          ? 'BLOCKED'
          : 'PENDING'
        : 'NOT_APPLICABLE',
...
for (const capability of contract.assuranceOnlyCapabilitySurfaces ?? []) {
  ...
  rows.push({
    componentId: `assurance::${capability}`,
    path: `ops/release/real-environment::${capability}`,
    type: 'assurance-surface',
    capability,
    executable: false,
    entrypoint: null,
    buildCommand: null,
    ...
    downstreamDependencies: [],
    owningPhases: [],
    realEnvironment: { ...evidence, evidenceRequired: true },
    evidenceId: null,
  });
}

const discoveredCapabilities = new Set(rows.map((row) => row.capability));
const missingCapabilities = contract.requiredCapabilitySurfaces.filter(
  (capability) => !discoveredCapabilities.has(capability),
);
const noAssurance = rows.filter(
  (row) => row.executable && row.realEnvironment.status === 'BLOCKED',
);
const blockedCanonicalPhaseDocs = rows.filter(
  (row) => row.type === 'roadmap-phase' && row.realEnvironment.status === 'BLOCKED',
);
const report = {
  schemaVersion: 3,
  generatedAt: new Date().toISOString(),
  commitSha: process.env.GITHUB_SHA ?? 'unknown',
  repositoryScope: contract.sourceOfTruth,
  componentCount: rows.length,
  sourceFileCount: rows.reduce((n, row) => n + row.sourceFiles, 0),
  components: rows,
  roadmap: {
    ...
    policy:
      'Only phase records explicitly designated as current/canonical by docs/phases/README.md are required. Historical 01–38 and future unstarted phases are not mechanically required.',
  },
  ...
  verdict:
    missingCapabilities.length || noAssurance.length || missingCanonicalPhaseDocs.length
      ? 'BLOCKED'
      : 'PASS',
  failures: [],
  blocked: [
    ...
    ...noAssurance.map(
      (row) =>
        `${row.path}: no registered real-environment assurance contract for capability ${row.capability}`,
    ),
    ...missingCapabilities.map(
      (capability) =>
        `${capability}: capability surface has no discovered repository owner or assurance-surface contract`,
    ),
  ],
...
console.log(
  `Components/phases: ${report.componentCount}; source files represented: ${report.sourceFileCount}`,
);
console.log(
  `Missing canonical phase docs: ${missingCanonicalPhaseDocs.length}; executable surfaces without assurance: ${noAssurance.length}`,
);
console.log(
  `Registered real-environment capability contracts: ${report.registeredRealEnvironmentContracts}`,
);

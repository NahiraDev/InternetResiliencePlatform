...
for (const file of files) {
  ...
  if (text.includes('<'.repeat(7)) || text.includes('>'.repeat(7)) || text.includes('='.repeat(7)))
...
for (const file of workflows) {
  ...
  if (!match) errors.push(`${rel} missing workflow name`);
  else if (workflowNames.has(match[1])) errors.push(`duplicate workflow name ${match[1]}`);
  else workflowNames.set(match[1], rel);

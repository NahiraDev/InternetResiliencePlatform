...
program
  ...
  .action(() => print({ status: runtime().state }));
program
  .command('config')
  .description('Print effective configuration')
  .action(() => print(loadConfig()));
program
  .command('providers')
  .description('List configured DNS providers')
  .action(() =>
    print(runtime().providers.map((p) => ({ id: p.id, name: p.name, metadata: p.metadata() }))),
  );
program
  .command('benchmark')
  .description('Run DNS benchmark')
  .action(async () => print(await runtime().benchmark.run(runtime().providers)));
program
  .command('metrics')

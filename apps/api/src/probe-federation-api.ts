...
const registration = z.object({
  probeId: z.string().regex(/^[a-zA-Z0-9._:-]{1,128}$/),
  name: z.string().min(1).max(256),
  region: z.string().min(1).max(256),
...
const evidence = z.object({
  payload: z.object({
    ...
    measurements: z.object({
      latencyMs: z.number().min(0).max(300000).optional(),
      jitterMs: z.number().min(0).max(300000).optional(),
      packetLossPercent: z.number().min(0).max(100).optional(),
      dnsLatencyMs: z.number().min(0).max(300000).optional(),
      tcpLatencyMs: z.number().min(0).max(300000).optional(),
      tlsLatencyMs: z.number().min(0).max(300000).optional(),
      httpLatencyMs: z.number().min(0).max(300000).optional(),
    }),
    metadata: z
      .record(z.string(), z.union([z.string().max(256), z.number().finite(), z.boolean()]))
      .optional(),
  }),
...
const query = z.object({
  ...
  probeId: z.string().max(128).optional(),
...
export interface ProbeFederationApiHandle {
  federation: ProbeFederation;
}

export const registerProbeFederationRoutes = (
  app: FastifyInstance,
  federation = new ProbeFederation(),
): ProbeFederationApiHandle => {
  ...
  app.get('/api/v1/federation/probes', async (request) => {
    await authz(request, 'runtime.inspect');
    return { success: true, data: federation.listProbes() };
  });
  app.post('/api/v1/federation/probes/:probeId/revoke', async (request) => {
    await authz(request, 'runtime.admin');
    const { probeId } = params.parse(request.params);
    return { success: true, data: { revoked: federation.revokeProbe(probeId) } };
  });
  ...
  app.get('/api/v1/federation/compare/:destination', async (request) => {
    await authz(request, 'runtime.inspect');
    const q = destination.parse({
      ...(request.params as { destination: string }),
      ...(request.query as object),
    });
    return { success: true, data: federation.compareDestination(q.destination, q.limit) };
  });
  app.get('/api/v1/federation/stats', async (request) => {
    await authz(request, 'runtime.inspect');
    return { success: true, data: federation.stats() };
  });
  return { federation };
};

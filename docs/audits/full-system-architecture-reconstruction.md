...
| Historical intelligence                          | `@irp/historical-analysis`, history API                                                            | historical-analysis                                       | PARTIALLY CONNECTED: no evidenced runtime decision input in the canonical cycle                                   |
...
| -------- | --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| HIGH     | duplicate / incorrect ownership         | `NetworkAutopilot` retains a historical state machine, diagnosis, policy evaluation, planning and action model. API and CLI compatibility surfaces now project `ResilienceRuntime` records rather than instantiate it; direct library consumers remain a compatibility risk. | `packages/resilience-runtime/src/autopilot/autopilot.ts`, `apps/api/src/index.ts`, `apps/cli/src/index.ts` | **CONTAINED.** `NetworkAutopilot` is deprecated and no production API/CLI caller instantiates it. Migrate remaining direct library consumers before removing the compatibility export. |
| HIGH     | missing integration                     | Historical analysis is consumed by API/history surfaces but has no evidenced input into `DecisionOrchestrator`/`CanonicalDecisionProvider`.                                                                                                                                  | historical-analysis, API, resilience-runtime                                                               | **OPEN.** Add an explicit advisory history port with freshness/confidence semantics; retain hard policy precedence.                                                                    |
...
The deterministic runtime E2E validation includes healthy, DNS degradation, provider recovery, and destination-specific scenarios; the Phase 78 integration test proves the bounded loop reaches the canonical runtime. These are controlled tests, not evidence that all adapters execute on a real host or that remote/federated paths work in production.

...
| MEDIUM   | historical-data scope                   | Historical analysis is an explicit read-only decision input and now filters evidence by destination, candidate, and optional provider/path/region/failure-domain metadata; a canonical graph-backed resource identity is still incomplete. | historical-analysis, resilience-runtime                                           | **PARTIALLY FIXED.** Scope is enforced at the historical and federated advisory adapters and is retained in ranking evidence. Complete graph-backed resource identity remains open. |

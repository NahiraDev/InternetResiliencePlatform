...
| Federation / remote intelligence                 | `ProbeFederation`, `FederatedEvidenceAdvisor`, federation API                                      | resilience-runtime federation advisory boundary                      | CONNECTED as an injected, signed-probe, destination-scoped advisory ranking input; federation failure is non-blocking |
...
| MEDIUM   | remote-resource scope                   | Accepted signed probe evidence is now normalized into canonical ranking, but remote nodes are not yet first-class path resources with capacity, provider, egress, and trust-score selection.                         | federation, resilience-runtime                                                    | **OPEN.** Extend the canonical path/resource model before allowing remote-node selection; keep remote evidence advisory.                  |
...
The deterministic runtime E2E validation includes healthy, DNS degradation, provider recovery, and destination-specific scenarios; the Phase 78 integration test proves the bounded loop reaches the canonical runtime. The advisory integration test proves retained DNS measurements and destination-scoped accepted probe evidence enter canonical ranking, while history-store or federation unavailability leaves the local decision path available. These are controlled tests, not evidence that all adapters execute on a real host or that remote/federated paths work in production.
